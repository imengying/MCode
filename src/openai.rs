...
mod tests {
    ...
    #[test]
    fn recognizes_compatible_context_overflow_errors() {
        let error = OpenAiError::Api {
            status: 400,
            body: "Input length (265330) exceeds model's maximum context length (262144)."
                .to_string(),
            request_id: None,
        };
        ...
        assert!(
            !OpenAiError::Api {
                status: 429,
                body: "too many tokens are being used; retry later".to_string(),
                request_id: None,
            }
            .is_context_overflow()
        );
